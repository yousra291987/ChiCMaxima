...
args=commandArgs(T)
if (length(args)==0) {
	cat(sprintf("usage: %s <repwindow> <output> <replicate1 interactions> <replicate2 interactions> [replicateX interactions...]\n",script.name))
	q("no")
}

repwindow=as.numeric(args[1])
ofile=args[2]
ifiles=args[3:length(args)]
if(length(ifiles)<3) {
...
flexwindow = function(gene,ints,repwindow) {
	...
	Ns=as.matrix(lookup)
	Enrichs=as.matrix(lookup)
	for (i in seq_along(as.numeric(colnames(lookup)))) {
		N.set=sub[[i]][lookup[,i],"N"]
		Ns[,i]=N.set
		enrich.set=sub[[i]][lookup[,i],"Enrichment"]
		Enrichs[,i]=enrich.set
		peak=cbind(peak,N.set,enrich.set)
		colnames(peak)[dim(peak)[2]-1]=paste0("N.",i)
		colnames(peak)[dim(peak)[2]]=paste0("Enrichment.",i)
	}
	peak$N.mean=rowMeans(Ns)
	peak$Enrichment.mean=rowMeans(Enrichs)
	return(peak)
}
...
for (i in 1:length(ifiles)) {
	...
	if(dim(tab)[2]!=12) {
		cat(paste("Wrong ibed format:",ifiles[i],"- expecting ID_Bait, chr_Bait, start_Bait, end_Bait, Bait_name, ID_OE, chr_OE, start_OE, end_OE, OE_name, N, Enrichment\n"))
	q("no")
	}
	ints[[i]]=tab
}
